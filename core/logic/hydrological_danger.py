...
class HydrologicalDangerRepository:
    ...
    @staticmethod
    async def update_hydrological_danger(pk: int | None, data: dict, event_pk: int | None) -> HydrologicalDanger | None:
        ...
        if event_pk is not None:
            updated = await HydrologicalDanger.objects.filter(event=event_pk).aupdate(**data)
        else:
        ...
        if event_pk is not None:
            return await HydrologicalDanger.objects.aget(event=event_pk)
...
class HydrologicalDangerService:
    ...
    async def update_hydrological_danger(self, data: dict, pk: int = None, event_id: int = None) -> ReturnDict:
        if pk is None and event_id is None:
            raise ValueError("Can't update without any ID key.")
        result = await self.repository.update_hydrological_danger(pk, data, event_id)
        if result is None:
